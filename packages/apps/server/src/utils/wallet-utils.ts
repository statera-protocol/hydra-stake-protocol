...
import { WalletContext } from "../types/common-types.js";
import {Contract, ImpureCircuitId} from "@midnight-ntwrk/compact-js";
...
import { DustWallet, UtxoWithMeta } from "@midnight-ntwrk/wallet-sdk-dust-wallet";
...
export async function buildWalletFromSeed(config: Config): Promise<WalletContext> {
  console.log(`Building wallet from genesis mint wallet seed...`);
  const WALLET_SEED = process.env.WALLET_SEED ?? null;
  if (WALLET_SEED == undefined || WALLET_SEED == null) throw new Error("Unable to find backend wallet seed");
  const seed = fromHex(WALLET_SEED);
...
`);
  ...
  // await ensureShieldedBalance(wallet, shieldedSecretKeys, dustSecretKey, unshieldedKeystore);
    ...
    await withStatus('Waiting for dust tokens to generate', () =>
      firstValueFrom(
        wallet.state().pipe(
          throttleTime(5_000),
          filter((s) => s.isSynced),
          filter((s) => s.dust.walletBalance(new Date()) > 0n),
        ),
      ),
    );
    return;
...
const buildShieldedAddress = (state: any): string => {
  const networkId = getNetworkId();
  const coinPubKey = ShieldedCoinPublicKey.fromHexString(state.shielded.coinPublicKey.toHexString());
  const encPubKey = ShieldedEncryptionPublicKey.fromHexString(state.shielded.encryptionPublicKey.toHexString());
...
export const configureProviders = async <TContract extends Contract<any, any>, PSID extends string>(ctx: WalletContext, config: Config, zkConfigPath: string):
Promise<DynamicProviders<TContract, PSID>> => {
  const walletAndMidnightProvider = await createWalletAndMidnightProvider(ctx);
  const zkConfigProvider = new NodeZkConfigProvider<ImpureCircuitId<TContract>>(zkConfigPath);
  return {
